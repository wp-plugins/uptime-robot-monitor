...
Description: Show you uptime stats from Uptime Robot on your wordpress pages, posts or in a widget.
...
echo "<th style=\"text-align: left;\">". __('Type', 'uptime-robot-nh')."</th>";
	} $GetUptimes = "&customUptimeRatio="; if(get_option('uptime_robot_nh_days_one','1') == 1){ $GetUptimes = $GetUptimes."1-";
...
if(get_option('uptime_robot_nh_monitors','775657602') == ""){
	$apiMonitors = "775657602";
}else{
	$apiMonitors = get_option('uptime_robot_nh_api','m775657602-b342a2e5f04a0e7707981fe8');
}
$url = "http://api.uptimerobot.com/getMonitors?apiKey=". $apiKey . "".substr($GetUptimes, 0, -1)."&format=xml&monitors=".$apiMonitors;
...
if($GetUptimes != "&customUptimeRatio="){
...
if(get_option('uptime_robot_nh_monitors','775657602') == ""){
	...
	$apiMonitors = get_option('uptime_robot_nh_monitors','m775657602-b342a2e5f04a0e7707981fe8');
}
$url = "http://api.uptimerobot.com/getMonitors?apiKey=". $apiKey . "&logs=1&format=xml&monitors=".$apiMonitors;
$xml = file_get_contents($url);
$xml = new SimpleXMLElement ($xml);
