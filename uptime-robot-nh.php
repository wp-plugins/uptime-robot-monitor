...
function uptime_robot_plugin_options() {
	...
	jQuery(document).ready(function($) {
		...
		localStorage.setItem("UrHash", div);
	    });
		window.history.pushState(null,null,localStorage.getItem("UrHash"));
		if(window.location.hash == "") { var hash = "ur-option-1"; } else { var hash = window.location.hash; }
		$(hash).show();
   		if (location.hash) {
 		 setTimeout(function() {
		    window.scrollTo(0, 0);
...
<div id="ur-option-3" class="wp-ur-set" style="display: none;">
   <?php } ?>
	<h3><?php _e('API connection settings', 'uptime-robot-nh'); ?></h3>
		...
		<td><span class="description"><?php _e('Copy your monitor ID\'s in this field seperated with a - (dash). You can find your monitor ID in your adressbar when the monitors dashboard is open. Leave empty to show all monitors in you account.', 'uptime-robot-nh'); ?></span></td>
            </tr>
        </table>

        <?php @submit_button(); ?>
</div>
<div id="ur-option-1" class="wp-ur-set" style="display: none;">
		...
		<td><input type="checkbox" name="uptime_robot_nh_thanks_2" id="uptime_robot_nh_thanks_2" value="0" <?php checked( '0', get_option( 'uptime_robot_nh_thanks_2' ) ); ?> > <span class="description"> <?php _e('Plugin by <a href="https://wordpress.org/plugins/uptime-robot-monitor/" target="_blank">Niels Hoogenhout.eu</a>', 'uptime-robot-nh'); ?></span> </td>
            </tr>
        </table>
        <table class="form-table">
...
<div id="ur-option-4" class="wp-ur-set" style="display: none;">
 	...
 	<?php echo '<input type="text" size="35" value="[uptime-robot-nh]" onclick="this.select()"> '.__('Use this shortcode on any page or post to display your uptime robot stats.', 'uptime-robot-nh'); ?><br>
 	<?php echo '<br><input type="text" size="35" value="[uptime-robot-nh id=00000-000001]" onclick="this.select()"> '.__('Show only specific monitors on any page or post to display your uptime robot stats. Seperate by a dash (-) to show multiple monitors.', 'uptime-robot-nh'); ?><br>
  	<?php echo '<br><input type="text" size="35" value="[uptime-robot-logs-nh]" onclick="this.select()"> '.__('Use this shortcode on any page or post to display your uptime robot logs.', 'uptime-robot--nh'); ?>
</div>
    </form>
